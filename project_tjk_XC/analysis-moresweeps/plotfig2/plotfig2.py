...
ax0.set_ylabel('$|P_{22}(x-x_0)|$', fontsize=14)
...
for ifile in range(nfile):
    ...
    ax0.loglog(x0,np.exp(popt[ifile][0])*x0**popt[ifile][1], '--'+mcolor[ifile],
...
ax1.set_ylabel('$P_{22}(x-x_0)/f(x-x_0)$',fontsize=14)
...
for ifile in range(nfile):
    ax1.plot(xdata[ifile],ydata_norm[ifile], '-'+mcolor[ifile], linewidth=0.1)
