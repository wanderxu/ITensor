  ...
  for Nx  in ${Nxarray}; do
    for J2 in ${J2array}; do
      ...
      for ((istep=$firststep;istep<=$maxstep;istep++)); do
        cd $WORKDIR
        datafile=$pretag/project_tjk_XC/run-moresweeps/${maindir}/step${istep}/paircorr.out
        #datafile=$pretag/project_tjk_XC/run-moresweeps/${maindir}/step${istep}/paircorrfixi0.out
        datafile2=$pretag/project_tjk_XC/run-moresweeps/${maindir}/step${istep}/pair_bubble.out
