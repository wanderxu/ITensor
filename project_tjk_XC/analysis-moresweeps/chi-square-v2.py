...
def curvefunc (xv, *p0 ):
        results = p0[0]*np.exp(-xv*p0[1]) + p0[2]*xv**(p0[3])
        return np.log(abs(results))
        ##results = p0[0]
...
##    print popt[i], "*x**", i
#print "log(abs(",popt[0],"*exp(-x*",popt[1],")+", popt[2], "*x**(",popt[3],")))"
#print("log(abs( %7.3f *exp(-x* %7.3f )+ %7.3f *x**( %7.3f )))".replace(" ","") % (popt[0], popt[1], popt[2], popt[3]))
tag=(" %7.3f *exp(-x* %7.3f )+ %7.3f *x**( %7.3f )" % (popt[0], popt[1], popt[2], popt[3]))
print tag.replace(" ", "")
#print " x^2 = ", rchi_sq
