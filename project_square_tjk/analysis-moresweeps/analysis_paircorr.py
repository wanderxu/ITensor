...
##calculate pairing
phase = np.zeros((9,6),dtype=complex)
phase[0,:] = [1.0+0.j, 1.0+0.j, 0.0+0.j, 1.0+0.j, 1.0+0.j, 0.0+0.j]/np.sqrt(4.0)
phase[1,:] = [1.0+0.j,-1.0+0.j, 0.0+0.j,-1.0+0.j, 1.0+0.j, 0.0+0.j]/np.sqrt(4.0)
phase[2,:] = [1.0+0.j,-1.0+0.j, 0.0+0.j, 1.0+0.j,-1.0+0.j, 0.0+0.j]/np.sqrt(4.0)
phase[3,:] = [1.0+0.j, 0.0+0.j, 0.0+0.j,-1.0+0.j, 0.0+0.j, 0.0+0.j]/np.sqrt(2.0)
phase[4,:] = [0.0+0.j,-1.0+0.j, 0.0+0.j, 0.0+0.j, 1.0+0.j, 0.0+0.j]/np.sqrt(2.0)
np.set_printoptions(precision=4,linewidth=400)
pairlist=['s', 'dxy', 'dx2-y2', 'px', 'py', '11', '12', '21', '22']
print "phi(", pairlist, ") = "
...
# calculate didj
# drop out boundaries
didj = np.zeros((9,N,N),dtype=complex)
for id1 in range(6):
    for id2 in range(6):
        for i in range(2*Ny,N-2*Ny): # note boundary data are dropped out
            for j in range(2*Ny,N-2*Ny):
                ...
                for ipair in range(3,5):
                # triplet pairing
                    didj[ipair,i,j] += np.conj(phase[ipair,id1])*phase[ipair,id2]*(sisj14[i,j,id1,id2] + sisj23[i,j,id1,id2])
for i in range(2*Ny, N-2*Ny):
    for j in range(2*Ny,N-2*Ny):
        # singlet pairing
        didj[5,i,j] += (sisj14[i,j,0,0] - sisj23[i,j,0,0])
        didj[6,i,j] += (sisj14[i,j,0,4] - sisj23[i,j,0,4])
        didj[7,i,j] += (sisj14[i,j,4,0] - sisj23[i,j,4,0])
        didj[8,i,j] += (sisj14[i,j,4,4] - sisj23[i,j,4,4])

# calculate dimj
dimj = np.zeros((9,N),dtype=complex)
for i in range(N):
    for j in range(N):
